...
// Roadmap-konform. Robust beim Auslesen der Select-Values.
// Persistenz: erstellt wins ggf. und macht manuelles Upsert (48h-Fenster).
...
export async function run(ctx) {
  try {
    ...
    // Prüfen auf Gleichstand in der Spitze
    ...
    // Gewinner persistieren – Tabelle sicherstellen + manuelles Upsert
    let winnerWinCount = (winner.wins ?? 0) + 1;
    let stored = false;
    try {
      await ensureWinsTable(db);

      // Transaktion: gibt es innerhalb 48h schon einen Eintrag für (guild,item,user)?
      ...
      if (exists?.length) {
        ...
        const { rows: upd } = await db.query(
          `
          UPDATE wins
          SET win_count = win_count + 1, roll_value = $4, rolled_at = NOW(), item_name_first = $5, reason = $6
          WHERE ctid IN (
            SELECT ctid FROM wins
            WHERE guild_id = $1
              AND item_slug = $2
              AND winner_user_id = $3
              AND rolled_at > NOW() - INTERVAL '48 hours'
